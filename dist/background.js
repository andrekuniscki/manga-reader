...
(() => {
  ...
  var pendingAutoActivate = /* @__PURE__ */ new Set();
  async function injectAndActivate(tabId) {
    await import_webextension_polyfill.default.scripting.executeScript({
      target: { tabId },
      files: ["dist/content.js"]
    });
    await import_webextension_polyfill.default.tabs.sendMessage(tabId, { type: "ACTIVATE_READER" });
  }
  import_webextension_polyfill.default.tabs.onUpdated.addListener((tabId, changeInfo) => {
  ...
  import_webextension_polyfill.default.runtime.onMessage.addListener(async (message, sender) => {
    ...
    if (msg?.type === "OPEN_URL_IN_READER") {
      const tab = await import_webextension_polyfill.default.tabs.create({ url: msg.url });
